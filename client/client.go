...
func (c *Config) CreateDatabase(db *CreateDatabaseConfig) error {
	var reply bool
	if db.AdminPassword == "" { 
		db.AdminPassword = c.AdminPassword
	}
	...
	err = client.Call("create_database", []interface{}{db.AdminPassword, db.DbName, db.Demo, db.Lang, db.UserPassword, db.Login, db.CountryCode }, &reply)
	if err != nil {
		return err
	}
	return err
...
func (c *Config) Read(model string, args []interface{}, options interface{}, elem interface{}) error {
	ne := elem.(Type).NilableType_()
	err := c.DoRequest("read", model, args, options, ne)
	if err == nil {
		reflect.ValueOf(elem).Elem().Set(reflect.ValueOf(ne.(NilableType).Type_()).Elem())
	}
	return err
}
...
func (c *Config) create(model string, fields map[string]interface{}, relation *Relations) (int64, error) {
	var id int64
	if relation != nil {
		HandleRelations(&fields, relation)
	}
	err := c.Create(model, []interface{}{fields}, &id)
	return id, err
}

func (c *Config) update(model string, ids []int64, fields map[string]interface{}, relation *Relations) error {
	if relation != nil {
		HandleRelations(&fields, relation)
	}
